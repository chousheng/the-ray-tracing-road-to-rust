...
```cpp filename="main.cc | ray_color() using a random ray direction"
...
```cpp filename="main.cc | Calculating reflected ray origins with tolerance"
...
```cpp filename="main.cc | ray_color() with replacement diffuse"
color ray_color(const ray& r, const hittable& world, int depth) {
    hit_record rec;
...
```cpp filename="main.cc | ray_color() with hemispherical scattering"
color ray_color(const ray& r, const hittable& world, int depth) {
    hit_record rec;
