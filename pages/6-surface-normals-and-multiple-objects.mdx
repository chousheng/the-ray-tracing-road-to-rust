...
```cpp filename="main.cc | Rendering surface normals on a sphere"
...
```cpp filename="main.cpp | Ray-sphere intersection code (before)"
double hit_sphere(const point3& center, double radius, const ray& r) {
    vec3 oc = r.origin() - center;
    auto a = dot(r.direction(), r.direction());
    auto b = 2.0 * dot(oc, r.direction());
...
```cpp filename="hittable.h | The hittable class"
#ifndef HITTABLE_H
#define HITTABLE_H
...
```cpp filename="hittable.h | Adding front-face tracking to hit_record"
struct hit_record {
    point3 p;
    vec3 normal;
    double t;
...
```cpp filename="hittable_list.h | The hittable_list class"
...
```cpp filename="rtweekend.h | The rtweekend.h common header"
#ifndef RTWEEKEND_H
#define RTWEEKEND_H
...
```cpp filename="main.cc | The new main with hittables"
#include "rtweekend.h"
