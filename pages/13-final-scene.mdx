...
Note that this tutorial is based on [version 3 of the first book](https://raytracing.github.io/v3/index.html). The original authors are working on version 4. In the new version, the final render result is the same, but they introduce a new `interval` class and move some code from the `main()` function to the `camera` class. Before you continue to follow the original series with version 4, you might want to implement those changes.

Finally, if you like this tutorial, leave a star on {<a href={config.project.link}>GitHub</a>}!
