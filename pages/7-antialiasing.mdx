...
<Callout emoji="🦀">
  Rust does not have a random number generator in the standard library. However, we can add an external library using Cargo. The code above uses the `rand` crate. We can install the crate by running `cargo add rand` in the terminal and Cargo will update the dependency in `Cargo.toml` for us.
</Callout>
...
<Img
  ...
  style={{ minWidth: "400px", imageRendering: "pixelated"}}
/>
