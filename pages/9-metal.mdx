...
The `HitRecord` is to avoid a bunch of arguments so we can stuff whatever info we want in there.  You can use arguments instead; it's a matter of taste.
...
What we have set up here is that `Material` will tell us how rays interact with the surface. `HitRecord` is just a way to stuff a bunch of arguments into a struct so we can send them as a group. When a ray hits a surface (a particular sphere for example), the material pointer in the `HitRecord` will be set to point at the material pointer the sphere was given when it was set up in `main()` when we start. When the `ray_color()` routine gets the `HitRecord` it can call methods of the material pointer to find out what ray, if any, is scattered.

<Callout emoji="🦀">
  The `Rc` smart pointer allow us to share ownership. Here we have to wrap `Option` around `Rc` because the corresponding C++ implementation uses `std::shared_ptr`, which by default initializes the value as `nullptr`. In Rust, `Rc` is not nullable. But we can use the `Option` enum to express the nullability while still achieving memory safety. In the Appendix, we will revisit the code and remove this `Option`.
</Callout>

To achieve this, we must have a pointer to the material for our `Sphere` struct to return within `HitRecord`. See the highlighted lines below:

<Tabs>
  <Tab label="Rust">
    ```rust filename="sphere.rs | Ray-sphere intersection with added Material information" {1-2,4,11,15,19,50}
    use std::rc::Rc;
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="material.rs | The Lambertian Material struct" {4,15-40}
    use crate::color::Color;
    use crate::hittable::HitRecord;
    use crate::ray::Ray;
    use crate::vec3;
  ...
  <Tab label="C++">
    ```cpp filename="material.h | The Lambertian Material class" {19-39}
    #ifndef MATERIAL_H
    #define MATERIAL_H
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="material.rs | Lambertian scatter, bullet-proof" {35-39}
    use crate::color::Color;
    use crate::hittable::HitRecord;
    use crate::ray::Ray;
    use crate::vec3;
  ...
  <Tab label="C++">
    ```cpp filename="material.h | Lambertian scatter, bullet-proof" {31-35}
    #ifndef MATERIAL_H
    #define MATERIAL_H
...
<Fig caption="Ray reflection" src="fig-1.11-reflection.jpg" label="reflection" />
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="vec3.rs | Vec3 reflection function" {182-185}
    use std::fmt::{Display, Formatter, Result};
    use std::ops::{Add, AddAssign, Div, DivAssign, Mul, MulAssign, Neg, Sub};
  ...
  <Tab label="C++">
    ```cpp filename="vec3.h | Vec3 reflection function" {151-154}
    #ifndef VEC3_H
    #define VEC3_H
...
<Callout emoji="🦀">
  Here, we use [`as_ref()`](https://doc.rust-lang.org/std/option/enum.Option.html#method.as_ref) to create a new `Option<&dyn Material>` value from the `Option<dyn Material>` owned by `rec.mat`. Then we use [`unwrap()`](https://doc.rust-lang.org/std/option/enum.Option.html#method.unwrap) to get the borrowed `&dyn Material`. Using `as_ref()` is necessary because `unwrap()` moves the `self` value. If we remove the `as_ref()` call, the compiler will say that `rec` cannot be used in the `scatter()` method as a whole because `rec.mat` has been moved by the `unwrap()` call.
</Callout>
...
<Img caption="Shiny metal" src="img-1.11-metal-shiny.png" />
...
<Fig
  ...
  src="fig-1.12-reflect-fuzzy.jpg"
  label="reflect-fuzzy"
/>
...
<Tabs>
  <Tab label="Rust">
    ```rust filename="material.rs | Metal Material fuzziness" {49,53-57,72}
    use crate::color::Color;
    use crate::hittable::HitRecord;
    use crate::ray::Ray;
    use crate::vec3;
  ...
  <Tab label="C++">
    ```cpp filename="material.h | Metal Material fuzziness" {49,60,66}
    #ifndef MATERIAL_H
    #define MATERIAL_H
...
<Img caption="Fuzzed metal" src="img-1.12-metal-fuzz.png" />
