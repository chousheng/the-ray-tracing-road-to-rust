...
Since we'll return either `Some(HitRecord)` or `None` in `Hittable::hit()`, we can remove `Option` and require all fields of the `HitRecord` struct to be initialized when creating. We can also remove its `Default` trait and the `new()` method:

```rust filename="hittable.rs | Updating HitRecord struct" {6,10,15-16}
use std::sync::Arc;
...
```rust filename="hittable_list.rs | Updating HittableList struct" {20-21,25-27,31}
...
```rust filename="sphere.rs | Updating Sphere struct" {25,32,42,46-52,55}
use std::sync::Arc;
...
```rust filename="material.rs | Change Material::scatter() to return Option<ScatterRecord>" {5-9,12}
...
```rust filename="material.rs | Updating Material structs" {26,34-37,56,58,60-67,91,111-114}
use crate::color::Color;
use crate::hittable::HitRecord;
use crate::ray::Ray;
use crate::{common, vec3};
...
```rust filename="main.rs | Updating ray_color()" {18,31-33}
mod camera;
mod color;
mod common;
mod hittable;
