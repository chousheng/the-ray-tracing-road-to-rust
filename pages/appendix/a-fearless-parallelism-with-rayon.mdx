...
We'll use [Rayon](https://github.com/rayon-rs/rayon), a data parallelism library, to parallelize the code and utilize all CPU cores of our machine.
...
1.  Use `into_par_iter()` to convert the `Range` into a `ParallelIterator`.

2.  Use `map()` with a closure specifying how the result of each pixel is calculated when iterating `i`. It will return another type implementing `ParallelIterator`.

3.  Use `collect()` to iterate, execute, and gather the results into a `Vec<Color>`.
...
However, it does not actually work yet. If you run `cargo check`, the compiler will complain:

```
error[E0277]: `(dyn Hittable + 'static)` cannot be shared between threads safely
```

The reason is that we have not made our code thread-safe yet.

One way to fix the error is to replace every occurrence of `dyn Hittable` with `dyn Hittable + Sync`, telling the compiler we will pass `Hittable` trait objects that also implement the `Sync` trait. However, this will make the code look repetitive all over the place.
...
<Callout emoji="🦀">
  `Send` and `Sync` are marker traits built into Rust's type system. Any type composed entirely of `Send` types is automatically `Send`. Same thing for the `Sync` trait.
</Callout>
